...
public class CoachController {
    ...
    public ResponseEntity<?> createCourseReminder(@PathVariable Integer courseId) {
        ...
        try {
            Date reminderDate = Date.from(course.getEndDate().minusDays(3).atStartOfDay(java.time.ZoneId.systemDefault()).toInstant()
            );
            calendarEventService.createCourseReminder(course.getName(), reminderDate);
            return ResponseEntity.status(200).body("Reminder added to Google Calendar");
        } catch (Exception e) {
